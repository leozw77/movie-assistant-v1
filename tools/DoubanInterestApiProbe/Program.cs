...
internal sealed class MainForm : Form
{
    ...
    private Control BuildLayout()
    {
        ...
        AddAutoRow(right, MakeButtonRow(_openHome, _checkLogin));
        AddAutoRow(right, new Label { Text = "豆瓣 Subject ID", AutoSize = true });
        AddAutoRow(right, _subjectId);
        AddAutoRow(right, new Label { Text = "状态", AutoSize = true });
        AddAutoRow(right, _status);

        var ratingRow = new FlowLayoutPanel { AutoSize = true, Dock = DockStyle.Fill, FlowDirection = FlowDirection.LeftToRight };
        ...
        AddAutoRow(right, ratingRow);

        AddAutoRow(right, _submitComment);
        right.RowStyles.Add(new RowStyle(SizeType.Absolute, 110));
        right.Controls.Add(_comment, 0, right.RowCount - 1);
        right.RowCount++;

        AddAutoRow(right, MakeButtonRow(_write, _remove));
        AddAutoRow(right, MakeButtonRow(_openSubject, _clearLog));
        AddAutoRow(right, new Label
        {
        ...
        right.Controls.Add(_log, 0, right.RowCount - 1);
    ...
    private async Task RemoveInterestAsync()
    {
        ...
        try
        {
            ...
            var fields = new Dictionary<string, string> { ["ck"] = auth.Ck! };
            var result = await PostFormAsync($"{MovieBase}/j/subject/{id}/removeinterest", id, auth, fields);
            LogHttpResult(result);
